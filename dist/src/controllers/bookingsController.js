...
exports.getRecentArtisanBookingsController = exports.BookingDetailsController = exports.completeBookingStatusController = exports.updateBookingStatusController = exports.getAllArtisanBookingsController = exports.getAllBookingsController = exports.createBookingsController = void 0;
const { Bookings, User, JobTypes } = require("../../models"); // adjust path if needed
const createBookingsController = async (req, res) => {
    try {
        const user = req.user;
        const { scheduledAt, location, notes, requestedBy, assignedArtisan, jobTypeKey, artisanStatus, status } = req.body;
        if (!scheduledAt)
            return res.status(400).json({ message: "Working day and Time is required." });
        if (!location)
        ...
        if (!assignedArtisan)
            return res.status(400).json({ message: "Assigned artisan is required." });
        if (!jobTypeKey)
...
exports.createBookingsController = createBookingsController;
...
const getAllArtisanBookingsController = async (req, res) => {
    try {
        const user = req.user;
        const bookings = await Bookings.findAll({
...
exports.getAllArtisanBookingsController = getAllArtisanBookingsController;
...
const completeBookingStatusController = async (req, res) => {
    try {
        const { id } = req.params; // search by key
...
exports.completeBookingStatusController = completeBookingStatusController;
...
const getRecentArtisanBookingsController = async (req, res) => {
    try {
        const user = req.user;
...
exports.getRecentArtisanBookingsController = getRecentArtisanBookingsController;
